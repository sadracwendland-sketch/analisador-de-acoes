...
def dataframe_to_records(df: pd.DataFrame) -> list[dict]:
    cols = [
        ...
        "sma10", "sma30", "rsi14", "macd", "macd_signal",
...
def run_full_analysis(ticker: str, period: str, capital: float, horizon: int) -> dict:
    ...
    # =========================
    try:
        ibov = provider.get_ibov_history(
            payload.daily_history.index.min(),
            payload.daily_history.index.max()
        )

        cdi = provider.get_cdi_daily(
            payload.daily_history.index.min(),
            payload.daily_history.index.max()
        )
    ...
    return {
        ...
        "benchmark": {
            "ibov_points": [
                {"date": idx.isoformat(), "close": round(float(val), 4)}
                for idx, val in ibov.tail(300).items()
            ],
            "cdi_daily": [
                {"date": idx.isoformat(), "rate": round(float(val), 6)}
                for idx, val in cdi.tail(300).items()
            ],
        },
        "meta": {
