...
def forecast_with_lstm(close_series: pd.Series, horizon: int = 7, epochs: int = 5, seq_len: int = 30) -> ForecastResult:
    # Limitar o histórico para os últimos 250 pontos para economizar memória no Render
    series = close_series.dropna().tail(250).astype(float)
    if len(series) < seq_len + 10 or torch is None or nn is None:
