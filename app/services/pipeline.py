...
def run_full_analysis(ticker: str, period: str, capital: float, horizon: int) -> dict:
    ...
    # Benchmarks (IBOV e CDI) podem falhar se o período for muito curto ou fins de semana
    try:
    ...
    return {
        ...
        "indicators": {
            "sma10": round(float(analysis_df["sma10"].iloc[-1]), 4) if pd.notna(analysis_df["sma10"].iloc[-1]) else None,
            "sma30": round(float(analysis_df["sma30"].iloc[-1]), 4) if pd.notna(analysis_df["sma30"].iloc[-1]) else None,
            "rsi14": round(float(analysis_df["rsi14"].iloc[-1]), 2) if pd.notna(analysis_df["rsi14"].iloc[-1]) else None,
            "macd": round(float(analysis_df["macd"].iloc[-1]), 4) if pd.notna(analysis_df["macd"].iloc[-1]) else None,
            "macd_signal": round(float(analysis_df["macd_signal"].iloc[-1]), 4) if pd.notna(analysis_df["macd_signal"].iloc[-1]) else None,
            "macd_hist": round(float(analysis_df["macd_hist"].iloc[-1]), 4) if pd.notna(analysis_df["macd_hist"].iloc[-1]) else None,
            "bb_upper": round(float(analysis_df["bb_upper"].iloc[-1]), 4) if pd.notna(analysis_df["bb_upper"].iloc[-1]) else None,
            "bb_mid": round(float(analysis_df["bb_mid"].iloc[-1]), 4) if pd.notna(analysis_df["bb_mid"].iloc[-1]) else None,
            "bb_lower": round(float(analysis_df["bb_lower"].iloc[-1]), 4) if pd.notna(analysis_df["bb_lower"].iloc[-1]) else None,
            "atr14": round(float(analysis_df["atr14"].iloc[-1]), 4) if pd.notna(analysis_df["atr14"].iloc[-1]) else None,
        },
        "signal": signal,
        "forecast": {
