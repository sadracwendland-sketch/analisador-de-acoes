...
def run_backtest(asset_daily: pd.DataFrame, ibov_close: pd.Series, cdi_daily: pd.Series) -> dict:
    ...
    return {
        "stats": {
            ...
            "strategy_return_pct": round((strat_curve.iloc[-1] - 1) * 100, 2),
            "ibov_return_pct": round((ibov_curve.iloc[-1] - 1) * 100, 2) if not ibov_curve.empty else 0.0,
            "cdi_return_pct": round((cdi_curve.iloc[-1] - 1) * 100, 2) if not cdi_curve.empty else 0.0,
        },
